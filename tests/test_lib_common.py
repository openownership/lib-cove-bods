...
class TestIsInterestCurrent():
    ...
    def test_this_YYYY(self):
        assert is_interest_current({'endDate': str(dateutil.utils.today(UTC).year)}) is True

    def test_this_YYYY_MM(self):
        today = dateutil.utils.today(UTC)
        assert is_interest_current({'endDate': '{}-{}'.format(today.year, today.month)}) is True

    def test_this_date(self):
        assert is_interest_current({'endDate': str(dateutil.utils.today(UTC).date())}) is True

    def test_yesterday(self):
        assert is_interest_current({'endDate': str(dateutil.utils.today(UTC).year-1)}) is False
