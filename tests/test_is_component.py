...
def test_iscomponent_true_but_has_component_statement_ids_1():
    ...
    assert results['additional_checks'][0]['statement'] == '12340547-d0c6-4a00-b559-5c5e91c34f5d'
...
def test_statement_is_component_but_is_not_used_1():
    ...
    cove_temp_folder = tempfile.mkdtemp(prefix='lib-cove-bods-tests-', dir=tempfile.gettempdir())
    json_filename = os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'fixtures', '0.2', 'statement_is_component_but_is_not_used_1.json'
    )
    ...
    assert results['additional_checks'][0]['type'] == 'statement_is_component_but_not_used_in_component_statement_ids'
    assert results['additional_checks'][0]['statement'] == '426b7dc8-8b7a-4d50-93de-8e200a4aedf5'
...
def test_statement_is_component_but_is_used_in_the_wrong_order_1():
    ...
    cove_temp_folder = tempfile.mkdtemp(prefix='lib-cove-bods-tests-', dir=tempfile.gettempdir())
    json_filename = os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'fixtures', '0.2', 'statement_is_component_but_is_used_in_the_wrong_order_1.json'
    )
    ...
    assert results['additional_checks'][0]['type'] == 'statement_is_component_but_is_after_use_in_component_statement_id'  # noqa
    assert results['additional_checks'][0]['statement'] == '24e604f2-790f-4600-8ff2-7fd8e46f047e'
    assert results['additional_checks'][0]['statement_type'] == 'ownership_or_control'
