...
def expected_errors():
    expected = []
    with open('tests/fixtures/0.4/invalid-schema/expected_errors.csv', newline='') as csvfile:
...
def test_all_schema_validation_invalid(expected_errors):
    ...
    for expected in expected_errors:
        ...
        json_filename = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "fixtures", "0.4", "invalid-schema", expected[0]
        )
        ...
        if expected[1] == 'required':
            assert results['validation_errors'][0]['extra']['required_key_which_is_missing'] == expected[3]
        elif expected[1] in ('enum', 'anyOf', 'type', 'maxLength', 'minLength',
                             'format', 'minimum', 'maximum', 'pattern', 'oneOf',
                             'const'):
            pass
        else:
            print(expected)
